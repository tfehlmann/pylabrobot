...
class EL406QueriesMixin:
  ...
  def _build_get_washer_manifold_command(self) -> bytes:
    """Build get washer manifold command bytes.

    Protocol format for get washer manifold:
      [0]   Command byte: 0xD8 (216)

    Returns:
      Command bytes.
    """
    return bytes([GET_WASHER_MANIFOLD_COMMAND])

  def _build_get_syringe_manifold_command(self) -> bytes:
    """Build get syringe manifold command bytes.

    Protocol format for get syringe manifold:
      [0]   Command byte: 0xBB (187)

    Returns:
      Command bytes.
    """
    return bytes([GET_SYRINGE_MANIFOLD_COMMAND])

  def _build_get_serial_number_command(self) -> bytes:
    """Build get serial number command bytes.

    Protocol format for get serial number:
      Command 256 (0x0100) is a 16-bit command sent in little-endian format.

      [0]   Command low byte: 0x00
      [1]   Command high byte: 0x01

    Returns:
      Command bytes.
    """
    return bytes([GET_SERIAL_NUMBER_COMMAND_LOW, GET_SERIAL_NUMBER_COMMAND_HIGH])

  def _build_get_sensor_enabled_command(self, sensor: EL406Sensor) -> bytes:
    """Build get sensor enabled command bytes.

    Protocol format for get sensor enabled:
      [0]   Command byte: 0xD2 (210)
      [1]   Sensor type: 0-5 (EL406Sensor value)
    ...
    Returns:
      Command bytes.
    """
    return bytes([GET_SENSOR_ENABLED_COMMAND, sensor.value])

  def _build_get_syringe_box_info_command(self) -> bytes:
    """Build get syringe box info command bytes.

    Protocol format for get syringe box info (may need verification with hardware):
      [0]   Command byte: GET_SYRINGE_BOX_INFO_COMMAND

    Returns:
      Command bytes.
    """
    return bytes([GET_SYRINGE_BOX_INFO_COMMAND])

  def _build_get_peristaltic_installed_command(self, selector: int) -> bytes:
    """Build get peristaltic installed command bytes.

    Note: This command uses a 16-bit command code (260 = 0x0104) unlike most
    other commands which use 8-bit codes. This is sent as a 16-bit value
    in little-endian format.

    Protocol format:
      [0]   Command low byte: 0x04
      [1]   Command high byte: 0x01
      [2]   Selector: 0 = primary, 1 = secondary

    Args:
      selector: Pump selector.
