...
class EL406ManifoldStepsMixin(EL406StepsBaseMixin):
  ...
  def _validate_aspirate_params(
    ...
    time_value, rate_byte = self._validate_aspirate_mode_params(
      vacuum_filtration, travel_rate, delay_ms, vacuum_time_sec,
    )
    self._validate_aspirate_offsets(
      offset_x, offset_y, offset_z,
      secondary_aspirate, secondary_x, secondary_y, secondary_z,
    )
  ...
  def _validate_wash_params(
    ...
    self._validate_wash_core_params(
      cycles, buffer, dispense_volume, dispense_flow_rate,
      dispense_x, dispense_y, dispense_z, aspirate_travel_rate,
      aspirate_z, pre_dispense_flow_rate, aspirate_delay_ms,
      aspirate_x, aspirate_y, wash_format, sector_mask,
    )
    self._validate_wash_final_and_extras(
      final_aspirate_z, final_aspirate_x, final_aspirate_y, final_aspirate_delay_ms,
      pre_dispense_volume, vacuum_delay_volume, soak_duration, shake_duration, shake_intensity,
    )
    self._validate_wash_secondary_aspirates(
      secondary_aspirate, secondary_z, secondary_x, secondary_y,
      final_secondary_aspirate, final_secondary_z, final_secondary_x, final_secondary_y,
    )
  ...
  def _encode_wash_byte_values(
    ...
    final_sec_y_byte = (
      encode_signed_byte(final_secondary_y) if final_secondary_aspirate else 0x00
    )
  ...
  def _build_wash_composite_command(
    ...
    disp1 = bytes(
      [
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  # [13-19] Padding (7 bytes)
    ...
    asp1 = bytes(
      [
        ...
        0x00, 0x00,  # [1-2] Delay ms LE (always 0 here)
        v["final_asp_z_low"],
        ...
        0x00, 0x00, 0x00, 0x00, 0x00,  # [13-17] Reserved (5 bytes)
    ...
    asp2 = bytes(
      [
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  # [11-18] Reserved
      ]
    )
    ...
    disp2 = bytes(
      [
        ...
        v["vac_delay_low"],
        v["vac_delay_high"],  # [11-12] Vacuum delay volume LE
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  # [13-18] Padding (6 bytes)
      ]
    )
    ...
    shake_soak = bytes(
      [
        ...
        0x00, 0x00, 0x00, 0x00,  # [7-10] padding
        0x00, 0x00, 0x00, 0x00,  # trailing padding (4 bytes)
      ]
    )
  ...
  def _build_wash_command(
    ...
    buffer: str,
    dispense_flow_rate: int = 7,
    dispense_offset_x: int = 0,
    dispense_offset_y: int = 0,
    dispense_offset_z: int = 120,
    aspirate_travel_rate: int = 3,
    aspirate_delay_ms: int = 5000,
    aspirate_offset_x: int = 0,
    aspirate_offset_y: int = 0,
    aspirate_offset_z: int = 29,
  ) -> bytes:
    """Build wash command bytes.
    ...
    Args:
      cycles: Cycle count.
      dispense_volume: Dispense volume in microliters.
      soak_time: Soak time in seconds.
      final_aspirate: Final aspirate flag.
      buffer: Buffer valve (A, B, C, D).
      dispense_flow_rate: Dispense flow rate (1-9).
      dispense_offset_x: Dispense X offset (signed, 0.1mm units).
      dispense_offset_y: Dispense Y offset (signed, 0.1mm units).
      dispense_offset_z: Dispense Z offset (0.1mm units).
      aspirate_travel_rate: Aspirate travel rate (1-9).
      aspirate_delay_ms: Aspirate delay in milliseconds.
      aspirate_offset_x: Aspirate X offset (signed, 0.1mm units).
      aspirate_offset_y: Aspirate Y offset (signed, 0.1mm units).
      aspirate_offset_z: Aspirate Z offset (0.1mm units).

    Returns:
      Command bytes.
    """
    vol_low, vol_high = encode_volume_16bit(dispense_volume)
    disp_z_low = dispense_offset_z & 0xFF
    disp_z_high = (dispense_offset_z >> 8) & 0xFF
    asp_delay_low = aspirate_delay_ms & 0xFF
    asp_delay_high = (aspirate_delay_ms >> 8) & 0xFF
    asp_z_low = aspirate_offset_z & 0xFF
    asp_z_high = (aspirate_offset_z >> 8) & 0xFF
    soak_min, soak_sec = encode_duration_mmss(soak_time)

    return bytes(
      [
        EL406StepType.M_WASH,
        cycles,
        buffer_to_byte(buffer),
        1 if final_aspirate else 0,
        vol_low,
        vol_high,
        dispense_flow_rate,
        encode_signed_byte(dispense_offset_x),
        encode_signed_byte(dispense_offset_y),
        disp_z_low,
        disp_z_high,
        aspirate_travel_rate,
        asp_delay_low,
        asp_delay_high,
        encode_signed_byte(aspirate_offset_x),
        encode_signed_byte(aspirate_offset_y),
        asp_z_low,
        asp_z_high,
        soak_min,
        soak_sec,
