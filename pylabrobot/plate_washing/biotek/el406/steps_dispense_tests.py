...
class TestEL406BackendDispense(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_dispense_validates_volume(self):
    ...
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=0.0)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=24.0)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=3001.0)

  async def test_dispense_validates_flow_rate(self):
    """Dispense should validate flow rate (1-11)."""
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, flow_rate=0)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, flow_rate=12)

  async def test_dispense_flow_rate_1_2_requires_vacuum_delay(self):
    """Flow rates 1-2 (cell wash) require vacuum_delay_volume > 0."""
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, flow_rate=1)  # no vacuum
    with self.assertRaises(ValueError):
  ...
  async def test_dispense_validates_offset_x(self):
    ...
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, offset_x=-61)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, offset_x=61)
  ...
  async def test_dispense_validates_offset_y(self):
    ...
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, offset_y=-41)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, offset_y=41)
  ...
  async def test_dispense_validates_offset_z(self):
    ...
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, offset_z=0)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, offset_z=211)

  async def test_dispense_validates_pre_dispense_volume(self):
    """Dispense should validate pre-dispense volume (0 or 25-3000)."""
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, pre_dispense_volume=10.0)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, pre_dispense_volume=3001.0)

  async def test_dispense_validates_pre_dispense_flow_rate(self):
    """Dispense should validate pre-dispense flow rate (3-11)."""
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, pre_dispense_flow_rate=2)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, pre_dispense_flow_rate=12)

  async def test_dispense_validates_vacuum_delay_volume(self):
    """Dispense should validate vacuum delay volume (0-3000)."""
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, vacuum_delay_volume=-1.0)
    with self.assertRaises(ValueError):
      await self.backend.dispense(volume=300.0, vacuum_delay_volume=3001.0)
  ...
  async def test_dispense_raises_when_device_not_initialized(self):
    ...
    with self.assertRaises(RuntimeError):
      await backend.dispense(volume=300.0)
