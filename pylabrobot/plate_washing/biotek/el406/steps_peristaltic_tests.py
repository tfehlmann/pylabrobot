...
class TestPeristalticPurgeCommandEncoding(unittest.TestCase):
  """Test peristaltic purge command binary encoding.

  Purge uses the same builder as prime (_build_peristaltic_prime_command)
  but is wrapped by _build_peristaltic_purge_command which sets the step type.

  Protocol format (shared with P_PRIME):
    [0]     plate type prefix (0x04=96-well)
    [1-2]   Volume (LE) — 0x0000 when use_volume=false
    [3-4]   Cycles (LE) when use_volume=false, else 0x0000
    [5]     Flow rate enum (0=Low, 1=Med, 2=High)
    [6]     Reverse/submerge (0/1)
    [7]     Cassette type
    [8]     Pump (1=Primary, 2=Secondary)
    [9-10]  Padding (0x0000)
  """
  ...
  def test_peristaltic_purge_prefix(self):
    """Peristaltic purge data should have plate type prefix (0x04=96-well) (step type in framing)."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=1000.0,
      flow_rate=2,
    )
    self.assertEqual(cmd[0], 0x04)

  def test_peristaltic_purge_volume_encoding(self):
    """Peristaltic purge should encode volume as little-endian 2 bytes."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=1000.0,
      flow_rate=2,
    )
  ...
  def test_peristaltic_purge_volume_500ul(self):
    """Peristaltic purge with 500 uL."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=500.0,
      flow_rate=2,
    )
    self.assertEqual(cmd[1], 0xF4)
    self.assertEqual(cmd[2], 0x01)

  def test_peristaltic_purge_volume_max(self):
    """Peristaltic purge with maximum GUI volume (3000 uL)."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=3000.0,
      flow_rate=2,
    )
    # 3000 = 0x0BB8 LE = [0xB8, 0x0B]
    self.assertEqual(cmd[1], 0xB8)
    self.assertEqual(cmd[2], 0x0B)

  def test_peristaltic_purge_flow_rate(self):
    """Peristaltic purge should encode flow rate at byte 5."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=1000.0,
      flow_rate=1,  # Medium
    )
    self.assertEqual(cmd[5], 1)

  def test_peristaltic_purge_flow_rate_high(self):
    """Peristaltic purge High flow rate → 2."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=1000.0,
      flow_rate=2,
    )
  ...
  def test_peristaltic_purge_command_length(self):
    """Peristaltic purge command should be 11 bytes."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=1000.0,
      flow_rate=2,
    )
  ...
  def test_peristaltic_purge_full_command(self):
    """Test complete peristaltic purge command."""
    cmd = self.backend._build_peristaltic_purge_command(
      volume=2000.0,
      flow_rate=1,
      reverse=False,
    ...
    self.assertEqual(cmd[1], 0xD0)  # Volume low byte (2000 = 0x07D0)
    self.assertEqual(cmd[2], 0x07)  # Volume high byte
    self.assertEqual(cmd[5], 1)  # Flow rate (Medium)
    self.assertEqual(cmd[6], 0)  # Reverse=False
    self.assertEqual(cmd[7], 2)  # Cassette 5uL=2
    self.assertEqual(cmd[8], 2)  # Pump Secondary=2

  def test_peristaltic_purge_same_as_prime(self):
    """Peristaltic purge data should be identical to prime data."""
    purge_cmd = self.backend._build_peristaltic_purge_command(
      volume=1000.0,
      flow_rate=2,
      reverse=True,
      cassette="Any",
    ...
    prime_cmd = self.backend._build_peristaltic_prime_command(
      volume=1000.0,
      flow_rate=2,
      reverse=True,
      cassette="Any",
    ...
    self.assertEqual(purge_cmd, prime_cmd)
