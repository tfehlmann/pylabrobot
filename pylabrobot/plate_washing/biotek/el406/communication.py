...
class EL406CommunicationMixin:
  ...
  async def _read_until(
    ...
    """Read from device until a specific character is received.

    Args:
      char: Character to read until (usually ACK_BYTE).
      timeout: Timeout in seconds (uses self.timeout if None).

    Returns:
      All bytes read including the terminating character.

    Raises:
      TimeoutError: If timeout is reached before character received.
      RuntimeError: If device responds with NAK (command rejected).
    """
    if timeout is None:
      timeout = self.timeout
    ...
    while True:
      try:
        ...
        raise EL406CommunicationError(
          f"Failed to read from device: {e}. Device may have disconnected.",
          operation="read",
          original_error=e,
        ) from e
      ...
      if byte:
        result += byte
        # Check for NAK only on FIRST byte
        if is_first_byte and byte[0] == NAK_BYTE:
          logger.error("Device returned NAK: %s", result)
          raise RuntimeError(
            f"Device rejected command (NAK). Response: {result!r}. "
            "This may indicate an invalid command, bad parameters, or device busy state."
          )
        is_first_byte = False
        if byte == char:
      ...
      if time.time() - t0 > timeout:
        logger.debug("Received incomplete response: %s", result)
        raise TimeoutError(f"Timeout waiting for response (received: {result!r})")
  ...
  async def _send_command(
    ...
    Uses async lock to prevent concurrent commands from interfering.
    ...
    Returns:
      Response bytes (ACK byte if wait_for_ack is True).
    ...
    async with self._command_lock:
      await self._purge_buffers()
      self._write_to_device(command)
      logger.debug("Sent: %s", command)

      if wait_for_ack:
        return await self._read_until(bytes([ACK_BYTE]), timeout=timeout)
      return b""
